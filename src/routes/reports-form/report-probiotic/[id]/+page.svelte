<script lang="ts">
	...
	let probioticCount = { m1: '', m2: '', m3: '', m4: '', m5: '', observation: '' };
	...
	async function fetchReport(retryCount = 0) {
		...
		try {
			const response = await fetch(`/api/probiotic-yogurt/${data.id}`, {
	...
	async function saveForm(retryCount = 0) {
		...
		const formData = {
			...
			probioticCountM1: parseFloat(probioticCount.m1) || 0,
			probioticCountM2: parseFloat(probioticCount.m2) || 0,
			probioticCountM3: parseFloat(probioticCount.m3) || 0,
			probioticCountM4: parseFloat(probioticCount.m4) || 0,
			probioticCountM5: parseFloat(probioticCount.m5) || 0,
			probioticCountObservation: probioticCount.observation || '',
	...
	async function deleteReport(retryCount=0) {
		...
		try {
			...
			if (response.ok) {
				...
				goto('/register/yogurt-probiotic-flavored');
			} else {
				const result = await response.json();
				if (response.status === 401 && result.reason === 'token_invalid' && retryCount < 1) {
					const refreshed = await authInstance.refreshToken();
...
<AuthGuard manual={true} forceLogin={true}>
	<div slot="authed" let:user>
		<section class="form-section">
			<h1>Informe de Yogur Probótico (ID: {data.id})</h1>
			<form on:submit|preventDefault={() => saveForm()}>
					...
					<table class="info-general-table">
						...
						<tbody>
							<tr>
								...
								<td><input type="text" bind:value={sample1.lot} /></td>
								<td><input type="text" bind:value={sample1.flavor} /></td>
								...
								<td
									><input
										type="number"
										step="0.1"
										bind:value={sample1.coldChamberTemperature}
									/></td
								>
							...
							<tr>
								...
								<td><input type="text" bind:value={sample2.lot} /></td>
								<td><input type="text" bind:value={sample2.flavor} /></td>
								...
								<td
									><input
										type="number"
										step="0.1"
										bind:value={sample2.coldChamberTemperature}
									/></td
								>
							...
							<tr>
								...
								<td><input type="text" bind:value={sample3.lot} /></td>
								<td><input type="text" bind:value={sample3.flavor} /></td>
								...
								<td
									><input
										type="number"
										step="0.1"
										bind:value={sample3.coldChamberTemperature}
									/></td
								>
							...
							<tr>
								...
								<td><input type="text" bind:value={sample4.lot} /></td>
								<td><input type="text" bind:value={sample4.flavor} /></td>
								...
								<td
									><input
										type="number"
										step="0.1"
										bind:value={sample4.coldChamberTemperature}
									/></td
								>
							...
							<tr>
								...
								<td><input type="text" bind:value={sample5.lot} /></td>
								<td><input type="text" bind:value={sample5.flavor} /></td>
								...
								<td
									><input
										type="number"
										step="0.1"
										bind:value={sample5.coldChamberTemperature}
									/></td
								>
					...
					<table class="results-table">
						...
						<tbody>
							<tr>
								...
								<td>UFC/g</td>
								<td><input type="number" bind:value={probioticCount.m1} /></td>
								<td><input type="number" bind:value={probioticCount.m2} /></td>
								<td><input type="number" bind:value={probioticCount.m3} /></td>
								<td><input type="number" bind:value={probioticCount.m4} /></td>
								<td><input type="number" bind:value={probioticCount.m5} /></td>
								<td><input type="text" bind:value={probioticCount.observation} /></td>
							</tr>
							<tr>
				...
				<div class="form-actions">
					...
					<button type="button" on:click={() => goto('/register/yogurt-probiotic-flavored')}
						>Regresar</button
					>
