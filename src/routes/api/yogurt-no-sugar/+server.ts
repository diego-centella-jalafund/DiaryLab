...
export const POST: RequestHandler = async ({ request }) => {
    try {
        ...
        const values = [
            ...
            formData.sng ? parseFloat(formData.sng) : null,
            formData.titratableAcidity ? parseFloat(formData.titratableAcidity) : null,
            formData.ph20C ? parseFloat(formData.ph20C) : null,
            formData.analysisTemperature ? parseFloat(formData.analysisTemperature) : null,
            ...
            formData.coliformCount ? parseInt(formData.coliformCount, 10) : null,
            formData.fecalColiforms ? parseInt(formData.fecalColiforms, 10) : null,
            formData.escherichiaColi || null,
            formData.moldYeastCount ? parseInt(formData.moldYeastCount, 10) : null,
            userId
        ];

        const result = await sql`
            INSERT INTO diarylab.non_sugar_yogurt (
