...
export async function GET({ request, url }) {
    try {
        ...
        const query = `
            SELECT sampling_date, titratable_acidity_m1, titratable_acidity_m2, titratable_acidity_m3, titratable_acidity_m4, titratable_acidity_m5
            FROM yogurt_fruited
            WHERE user_id = $1
        ...
        try {
            ...
            const transformedData = result.rows.map((row) => ({
                ...
                titratableAcidityM4: row.titratable_acidity_m4 || 0,
                titratableAcidityM5: row.titratable_acidity_m5 || 0
            }));
