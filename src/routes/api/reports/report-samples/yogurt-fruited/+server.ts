    ...
    try {
        ...
        try {
            const result = await sql`
                SELECT sampling_date, titratable_acidity_m1, titratable_acidity_m2, titratable_acidity_m3, 
                       titratable_acidity_m4, titratable_acidity_m5
            ...
            const transformedData = result.map((row: any) => ({
                sampling_date: row.sampling_date.toISOString().split('T')[0],
                ...
                titratableAcidityM4: row.titratable_acidity_m4 ?? null,
                titratableAcidityM5: row.titratable_acidity_m5 ?? null,
            }));
